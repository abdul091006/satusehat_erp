...
public class SatuSehatCekNIK {
    ...
    private static final Map<String, String> CACHE_ID_PASIEN = new HashMap<String, String>();
    private static final Map<String, String> CACHE_ID_PRAKTISI = new HashMap<String, String>();
    ...
    public String tampilIDPasien(String cari) {
        idpasien="";
        if(!isValidNik(cari)){
            System.out.println("Lewati cek Patient SatuSehat, NIK tidak valid : "+cari);
            return idpasien;
        }
        if(CACHE_ID_PASIEN.containsKey(cari)){
            return CACHE_ID_PASIEN.get(cari);
        }
        try{
            headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
        ...
        CACHE_ID_PASIEN.put(cari, idpasien);
        return idpasien;
    }
    ...
    public String tampilIDParktisi(String cari) {
        idpasien="";
        if(!isValidNik(cari)){
            System.out.println("Lewati cek Practitioner SatuSehat, NIK tidak valid : "+cari);
            return idpasien;
        }
        if(CACHE_ID_PRAKTISI.containsKey(cari)){
            return CACHE_ID_PRAKTISI.get(cari);
        }
        try{
            headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
        ...
        CACHE_ID_PRAKTISI.put(cari, idpasien);
        return idpasien;
    }

    private boolean isValidNik(String cari) {
        return cari != null && cari.matches("\\d{16}");
    }
}
