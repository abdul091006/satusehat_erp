frappe.ui.form.on("Khanza SatuSehat Sync Record", {
	refresh(frm) {
		...
		if (frm.doc.import_status === "imported" && frm.doc.approval_status !== "Approved") {
			frm.add_custom_button(__("Approve & Queue Send"), () => {
				frappe.call({
					method: "healthcare.healthcare.khanza_main.approve",
					args: { queue_name: frm.doc.name },
					freeze: true,
					freeze_message: __("Approving and queueing SatuSehat send..."),
		...
		if (frm.doc.import_status === "imported" && frm.doc.approval_status !== "Rejected" && frm.doc.sync_status !== "sent") {
			frm.add_custom_button(__("Needs Correction"), () => {
				frappe.prompt(
					...
					(values) => {
						frappe.call({
							method: "healthcare.healthcare.khanza_main.needs_correction",
							args: { queue_name: frm.doc.name, reason: values.reason },
							callback: () => frm.reload_doc(),
						});
					},
					__("Mark SatuSehat Sync Record as Needs Correction")
				);
			});
		}
